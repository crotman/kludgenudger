...
synthetic_predictions <- read_feather("predictions.rds")
...
plot_bags <- girafe(ggobj = plot_bags, width_svg = 9, height_svg = 5 )
  


renderGirafe(plot_bags)
...
median_prevalence_bags <- median(plot_data_bags$prop_satd)
...
plot_2_bags <- ggplot( 
  ...
  geom_hline_interactive(
    yintercept = median_prevalence_bags,
    color = "darkblue",
    tooltip = paste0("Median proportion: ", percent(median_prevalence_bags, accuracy = 0.1) )
  ) +
  coord_flip() +
  scale_y_continuous(labels = percent_format(accuracy = 1)) +
  theme_minimal() +
...
plot_2_bags <- girafe(ggobj = plot_2_bags, width_svg = 9, height_svg = 5 )


renderGirafe(plot_2_bags)
...
modelo_bags <- read_rds("modelo_bags.rds")
...
synthetic_predictions_bags <- read_feather("predictions_bags.rds")

plot_reg_bags <- ggplot(
  synthetic_predictions_bags
) +
  geom_line(
    aes(
      y = prediction,
...
```{r}
# plot_violin_bags <- data_regression_bags %>% 
#   mutate(
#     has_satd = if_else(has_satd, "SATD", "NO SATD"),
#     has_alert = if_else(has_alert, "PMD ALERT", "NO PMD ALERT"),
#   ) %>% 
...
#     theme_minimal()
# 
# plot_violin_bags <- girafe(
#   ggobj = plot_violin_bags, 
#   width_svg = 9, 
#   height_svg = 5
# 
# ) %>% 
